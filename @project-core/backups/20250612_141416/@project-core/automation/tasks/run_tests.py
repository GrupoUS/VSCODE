...
try:
    ...
    from helpers.core.argument_parser import create_test_parser
...
if HELPERS_AVAILABLE:
    logger = get_vibecode_logger("run_tests")
else:
    logger = logging.getLogger(__name__)
...
class TestRunner:
    ...
    def _test_memory_validation(self) -> bool:
        ...
        if master_rule.exists():
            try:
                content = master_rule.read_text(encoding='utf-8')
            except UnicodeDecodeError:
                # Fallback para encoding alternativo
                content = master_rule.read_text(encoding='latin-1')
...
def main():
    ...
    if HELPERS_AVAILABLE:
        log_execution_start(logger, "run_tests", vars(args))
    ...
    if HELPERS_AVAILABLE:
        log_step(logger, f"Executando testes nível {args.level}", "start")
    ...
    if args.report:
        if HELPERS_AVAILABLE:
            log_step(logger, "Gerando relatório", "start")
    ...
    if HELPERS_AVAILABLE:
        ...
        log_execution_end(logger, "run_tests", success, duration)
