...
def create_incremental_backup(file_paths: List[Path], description: str) -> bool:
    ...
    try:
        ...
        for path in file_paths:
            ...
            # Use Smart Backup System V4.0 for safe backup operations
            try:
                import subprocess
                smart_backup_result = subprocess.run([
                    "powershell", "-Command",
                    f"& '@project-core/automation/smart-backup-system-v4.ps1' -SourcePath '{path}' -BackupName 'auto-backup-{path.name}' -MaxSizeMB 50 -DryRun:$false"
                ], capture_output=True, text=True, check=False)

                if smart_backup_result.returncode == 0:
                    log_status(f"  ✅ Smart backup created: {path}", "success")
                    if path.is_dir():
                        size = sum(f.stat().st_size for f in path.rglob('*') if f.is_file())
                    else:
                        size = path.stat().st_size
                else:
                    # Fallback with size validation
                    if path.is_dir():
                        total_size = sum(f.stat().st_size for f in path.rglob('*') if f.is_file())
                        if total_size > 50 * 1024 * 1024:  # 50MB limit
                            log_status(f"  🚫 Skipped oversized directory: {path} ({total_size / (1024*1024):.1f} MB)", "warning")
                            continue
                        shutil.copytree(path, destination_path, dirs_exist_ok=True)
                        size = total_size
                    else:
                        if path.stat().st_size > 50 * 1024 * 1024:  # 50MB limit
                            log_status(f"  🚫 Skipped oversized file: {path} ({path.stat().st_size / (1024*1024):.1f} MB)", "warning")
                            continue
                        shutil.copy2(path, destination_path)
                        size = path.stat().st_size
            except Exception as e:
                log_status(f"  ⚠️ Smart backup failed for {path}, using fallback: {e}", "warning")
                # Safe fallback with size validation
                if path.is_dir():
                    total_size = sum(f.stat().st_size for f in path.rglob('*') if f.is_file())
                    if total_size > 50 * 1024 * 1024:  # 50MB limit
                        log_status(f"  🚫 Skipped oversized directory: {path} ({total_size / (1024*1024):.1f} MB)", "warning")
