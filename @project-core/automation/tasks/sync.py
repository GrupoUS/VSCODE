...
try:
    ...
    from helpers.system.process_utils import run_git_command, check_git_status, git_add_commit_push
...
if HELPERS_AVAILABLE:
    logger = get_vibecode_logger("sync")
else:
    logger = logging.getLogger(__name__)
...
class SyncManager:
    ...
    def __init__(self, project_root: str = None):
        ...
        if HELPERS_AVAILABLE:
            ...
            self.backup_manager = BackupManager(self.paths.backups_path)
        else:
            # Fallback para compatibilidade
    ...
    def quick_push(self, message: str = None) -> bool:
        ...
        """
        log_step(logger, "⚡ Executando push rápido...")
        ...
        if HELPERS_AVAILABLE:
            ...
            if result.success:
                ...
                logger.error(f"❌ Erro no push rápido: {result.stderr}")
                return False
        else:
    ...
    def _sync_project(self, project_path: Path, force: bool = False,
        ...
        try:
            if HELPERS_AVAILABLE:
                # Usar helpers Git
                fetch_result = run_git_command(['fetch'], cwd=project_path)
                if not fetch_result.success:
                    logger.error(f"❌ Erro no fetch: {fetch_result.stderr}")
                    return False
    ...
    def _create_backup(self) -> bool:
        ...
        if HELPERS_AVAILABLE and self.backup_manager:
            ...
            backup_name = f"sync_backup_{timestamp}"

            backup_path = self.backup_manager.create_backup(
                source=self.project_core,
                backup_name=backup_name,
                exclude_patterns=["*/backups/*", "*.tmp", "*.log"]
            )
            ...
            if backup_path:
                ...
                logger.error("❌ Falha ao criar backup com BackupManager")
                return False
        else:
            # Fallback manual
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            try:
                ...
                logger.info(f"💾 Backup manual criado: {backup_dir}")
                return True
...
def main():
    ...
    if HELPERS_AVAILABLE:
        parser = VibeCodeArgumentParser("sync", 'VIBECODE Sync Manager V4.0')

        # Adicionar argumentos específicos de sync
        parser.add_custom_argument('--target', choices=['all', 'specific', 'existing'],
                                 default='all', help='Alvo da sincronização')
        parser.add_custom_argument('--force', action='store_true', help='Forçar sincronização (sobrescrever alterações locais)')
        parser.add_custom_argument('--auto-push', action='store_true', help='Push automático após sincronização')
        parser.add_custom_argument('--projects', nargs='+', help='Lista de projetos específicos para sincronizar')
        parser.add_custom_argument('--quick-push', action='store_true', help='Executar push rápido do repositório atual')
        parser.add_custom_argument('--final-push', action='store_true', help='Executar push final em todos os projetos')
        parser.add_custom_argument('--unified', action='store_true', help='Sincronizar ambiente unificado')
        parser.add_custom_argument('--status', action='store_true', help='Mostrar status de sincronização')
        parser.add_custom_argument('--message', type=str, help='Mensagem de commit personalizada')
        parser.add_custom_argument('--project-root', type=str, help='Caminho raiz do projeto')

        args = parser.parse_args()

        # Log de início de execução
        log_execution_start(logger, "sync", vars(args))
    else:
        ...
        parser.add_argument('--target', choices=['all', 'specific', 'existing'],
                           default='all', help='Alvo da sincronização')
        parser.add_argument('--force', action='store_true', help='Forçar sincronização (sobrescrever alterações locais)')
        parser.add_argument('--auto-push', action='store_true', help='Push automático após sincronização')
        parser.add_argument('--projects', nargs='+', help='Lista de projetos específicos para sincronizar')
        parser.add_argument('--quick-push', action='store_true', help='Executar push rápido do repositório atual')
        parser.add_argument('--final-push', action='store_true', help='Executar push final em todos os projetos')
        parser.add_argument('--unified', action='store_true', help='Sincronizar ambiente unificado')
        parser.add_argument('--status', action='store_true', help='Mostrar status de sincronização')
        parser.add_argument('--message', type=str, help='Mensagem de commit personalizada')
    ...
    if HELPERS_AVAILABLE:
        ...
        log_execution_end(logger, "sync", success, duration)
